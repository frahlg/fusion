...
#
# Notes:
# - Model + provider are inherited from the user's ~/.codex/config.toml (e.g. gpt-5.5). We only force the
#   reasoning effort to xhigh so a Fusion panelist always thinks at full depth, regardless of the config default.
...
# - Exit 127 means the codex CLI isn't installed — the orchestrator drops this panelist and downgrades the panel.
...
echo "[run_codex.sh] ok -> $output_file (effort=$effort)"
