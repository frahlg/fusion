...
editing it. It is *not* a system prompt and is *not* installed into `~/.claude` — only `skills/fusion/` and
`commands/` are copied by `install.sh`.
...
```
install.sh                              # copies skill + command into ~/.claude (CLAUDE_CONFIG_DIR override)
commands/fusion.md                      # /fusion <question>
...
skills/fusion/scripts/detect_panel.sh   # detect codex/gemini, recommend the richest panel
...
- **Opus 4.8 always judges and writes the final answer** — the pipeline can't be reversed (panelist models
  can't call back out to spawn Opus). The slug reads driver-first for that reason.
- Panelists must stay **isolated**: never paste one panelist's output into another's prompt.
- `run_codex.sh` inherits model + provider from `~/.codex/config.toml` and only forces
